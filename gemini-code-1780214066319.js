 ...
 * Version: Ultimate Failproof Build (Strict Mode + Exact Match CT)
 */
...
function getIndividualClasses(classNameStr) {
    ...
    groups.forEach(group => {
        ...
        // Fallback in case of missing hyphens in some other place
        if (parts.length < 2) {
...
window.syncFromCloud = async function() {
    ...
    try {
        ...
        try {
            ...
            throw new Error("Invalid JSON response. Ensure AppScript returns valid JSON.");
        }
        ...
        window.rawAssignmentsData.slice(1).forEach(row => {
            ...
            blocks.forEach(block => {
                ...
                distinctClasses.forEach(distinctClassGroup => {
                    ...
                    if (finalPeriods > 0) {
                        // 🌟 FIX: Bulletproof EXACT MATCH Class Teacher Logic
                        let isCT = false;
